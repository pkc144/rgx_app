...
| Kotak | Credential | ucc, apiKey (API Access Token), mobile, mpin, totp | ~1h | No | Single UUID "API Access Token" from NEO → TradeAPI → API Dashboard. TOTP required on every reconnect. **No separate Consumer Secret** — aligned with web 2026-04-22. All downstream payloads (`kotak/order-book`, `kotak/order-cancel`, `kotak/positions`, `kotak/holdings`, `kotak/all-holdings`, `kotak/funds`, rebalance, ProcessTrades, MP/Bespoke performance, basket run) send `consumerKey` (= UUID) + `accessToken` + `viewToken` + `sid` + `serverId` + `baseUrl`; **no `consumerSecret`** field is sent anywhere. The ccxt `/kotak/v2/modify-order` endpoint was also realigned 2026-04-22 to match this shape (previously read `credentials.secretKey` and had inverted field mapping — see CHANGELOG 3.9.5). **Per-DC `baseUrl`**: Kotak NEO returns a per-user data-center URL (e.g. `https://e43.kotaksecurities.com`) in its validate response. Without it ccxt-india's `Kotak.__init__` falls back to `gw-napi.kotaksecurities.com` which 502's empty for DC-bound endpoints (orders, holdings, positions, funds, scripmaster, user-limits). The backend persists `baseUrl` to BOTH top-level `user.baseUrl` AND `connected_brokers[Kotak].baseUrl` (slot persistence wired 2026-04-30 via `Models/userModel.js` sub-schema field + `MultiBrokerService.addBrokerConnection` copy — pre-fix mongoose strict mode silently dropped the field). ccxt-india's Kotak client now fail-fasts with `error: "baseurl_missing"` for DC-required routes when `base_url` is None, prompting the user to reconnect Kotak rather than surfacing a generic 502. |
