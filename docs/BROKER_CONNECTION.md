...
| IIFL Securities | **OAuth (WebView)** | clientCode, jwtToken (sessionToken) | Session | **Yes** | **Re-wired 2026-06-09 (web parity, Stage 1).** Opens `https://markets.iiflcapital.com/?v=1&appkey=<APPKEY>&redirect_url=<HOST>`, WebView intercepts `?authcode=&clientid=` on callback, exchanges via `POST {ccxt}/iifl/login/client` → returns `sessionToken` persisted via backend. `appkey` defaults to platform-wide `nHjYctmzvrHrYWA` (overridable via `REACT_APP_IIFL_APP_KEY`). Modal at `src/components/iiflmodal.js` was previously imported but missing the `server` import — fixed in the same commit. |
